...
def _generate_video_tool(params: dict) -> str:
    try:
        ...
        if isinstance(raw, str):
            ...
            paths = []
        output_path = (params.get("output_path") or "outputs/agent_slideshow.mp4").strip()
        fps = float(params.get("fps", 1.0) or 1.0)
        if not paths:
            return "generate_video error: missing image_paths (comma-separated or JSON array)"
        return json.dumps(
            generate_video(GenVideoBody(image_paths=paths, output_path=output_path, fps=fps)),
            ensure_ascii=False,
        )
    except Exception as e:
...
async def run_agent(message: str, model: str):
    ...
    remember_from_message("agent", "user", message)
    ...
    for _ in range(rt.agent_max_steps):
        if forced_tool_call is not None and not tool_used:
            ...
            try:
                ...
                if not _looks_like_garbled_text(message):
                    remember_from_message("agent", "assistant", cleaned)
        ...
        if tool_call:
            ...
            if result_failed:
                ...
                remember_from_message("agent", "assistant", final_text)
...
async def agent_run(req: AgentRequest):
    ...
    async def generate():
        try:
            for step in await run_agent(req.message, req.model):
