...
def generate_video(body: GenVideoBody) -> dict[str, Any]:
    ...
    # MoviePy 要求所有帧尺寸一致；这里按第一张图统一到同一尺寸。
    normalized_paths = paths
    temp_files: list[Path] = []
    if Image is not None and paths:
        try:
            ...
            for idx, src in enumerate(paths):
                ...
                tmp = OUTPUTS_DIR / f"_video_frame_{idx}.png"
                img.save(tmp)
                normalized_paths.append(str(tmp))
    ...
    out = ROOT / body.output_path
    out.parent.mkdir(parents=True, exist_ok=True)
    clip = ImageSequenceClip(normalized_paths, fps=body.fps)
    write_kw: dict[str, Any] = {"codec": "libx264", "audio": False}
    try:
