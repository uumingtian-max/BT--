...
- **Ollama 路线（推荐）**：`backend\.env` + `backend\.env.example`（五模型岗位，见 `model_pin_roles` 技能）
- **vLLM/llama.cpp 路线**：`backend\.env.local-llamacpp.example`，模型 ID 须与 `/v1/models` 一致
- GitHub：[uumingtian-max/ai-agent-project](https://github.com/uumingtian-max/ai-agent-project)

### Ollama 岗位（5090 24G：4 常驻 + 1 按需）

| 模型 | 职责 | VRAM |
|------|------|------|
| `nomic-embed-text` | 嵌入 | 常驻 ~0.3G |
| `functiongemma` | 工具路由 | 常驻 ~0.3G |
| `qwen3.5:4b` | 答案 / 视觉 / 体检 JSON / 简单代码 | 常驻 ~3.4G |
| `deepseek-r1:7b` | 推理 / 规划 / 进化 | 常驻 ~4.7G |
| `deepseek-coder-v2:16b` | 复杂写码 | **按需** ~8.9G，用完 unload |
| `generate_image` | 画图（SD） | 非 Ollama |

`OLLAMA_RESIDENT_MODELS` / `OLLAMA_ON_DEMAND_MODELS` 见 `backend/.env.example`。

管理脚本：`scripts\MANAGE_MODELS.ps1`、`scripts\pin-ollama-models.ps1`
...
若 push 后 CI 全红且日志写 **account is locked due to a billing issue**，属于 GitHub 账户/额度问题，与代码无关；需在 GitHub → Settings → Billing 恢复后再重跑 workflow。
...
- 重要优化后提交并同步 GitHub。
- 不提交 `.env`、数据库、`node_modules`、`outputs`、`frontend/build`、`archive/_onyx_pack_*`（整包快照）、`vendor/*/` 克隆体、个人运行记录、临时文件。
- 接手本地仓库前先看 `git status` 和 `git diff`，尤其注意 `backend/context_pack.py`、`backend/memory_store.py` 可能有未提交半成品。

## 已统一修正（维护补丁）

- `backend/.env.example`：去掉重复 `AGENT_DEFAULT_MODEL`、Ollama/vLLM 分节、F5-TTS 全变量、五模型岗位
- `backend/model_router.py`：与精简栈一致；简单代码 → `CODE_SIMPLE_MODEL`，复杂 → `CODE_MODEL`
- `package.json`：Electron `^36.4.0`
- `.github/workflows/ci.yml`：测试模型 `qwen3.5:4b`；审计 `backend/requirements.txt`
- `scripts/MANAGE_MODELS.ps1`：拉取 / 预热 / 删重叠模型

## 下一阶段优先级

1. 品牌统一：对外统一 **BT（黑光）**（README、启动器、窗口标题、manifest、图标）。
2. 模型 ID 统一：Ollama 看 `ollama list`；vLLM 看 `.env` 与 `/v1/models` 一致。
...
- 默认只拉 2 个对照仓：**agency-agents**、**gstack**（`.\scripts\sync-bt-vendor-repos.ps1`）
- 清单与说明：`vendor/repos.manifest.json`、`vendor/README.md`
- Agent 技能：`bt_external_repos`；勿对 BT 执行 Hermes `skills install`
