...
def get_system_prompt(user_input: str = "", private_mode: bool = False) -> str:
    ...
    if soul: parts.append("【人格核心·SOUL】\n" + soul)
    user_profile = _read_md(USER_PATH)
    if user_profile: parts.append("【用户画像】\n" + user_profile)
    ...
    if files:
        ...
        for f in files[:4]: lines.append(f"  - {f['name']}：{f['summary'][:120]}")
        parts.append("\n".join(lines))
    if private_mode or is_private_mode(user_input):
        priv = _read_md(USER_PRIVATE_PATH)
        if priv: parts.append("【私密人格层·已解锁】\n" + priv)
    return "\n\n".join(parts)
