...
export default function AutomationDashboard() {
  ...
  const selectedRunRef = React.useRef('');

  React.useEffect(() => {
    selectedRunRef.current = selectedRunId;
  }, [selectedRunId]);

  const refresh = React.useCallback(async (pinRunId) => {
    setError('');
    try {
      ...
      const preferredRunId = pinRunId || selectedRunRef.current;
      const runId = preferredRunId || nextGraphs[0]?.id || '';
  ...
  React.useEffect(() => {
    refresh();
    const timer = setInterval(() => refresh(selectedRunRef.current), 10000);
