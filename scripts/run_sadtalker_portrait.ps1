...
# 用法: cd ai-agent-project; .\scripts\run_sadtalker_portrait.ps1
...
if ($py.Mode -eq "active") {
    & python "$Root\scripts\fix_sadtalker_env.py"
} else {
    & conda run -n $SadEnv --no-capture-output python "$Root\scripts\fix_sadtalker_env.py"
}
